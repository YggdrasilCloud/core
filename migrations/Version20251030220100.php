...
final class Version20251030220100 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_photos_file_name ON photos(file_name)');

        // Index for filtering by MIME type (image/jpeg, image/png, etc.)
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_photos_mime_type ON photos(mime_type)');

        // Index for sorting/filtering by file size
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_photos_size_in_bytes ON photos(size_in_bytes)');

        // Index for sorting/filtering by upload date
        $this->addSql('CREATE INDEX IF NOT EXISTS idx_photos_uploaded_at ON photos(uploaded_at)');
    }
